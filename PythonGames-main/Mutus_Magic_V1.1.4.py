...
# Mutus Magic is a Text-Turn-based Role Playing Game (TTBRPG) with a GUI using Tkinter.


#TODO not in ORDER
"""
- Add a merchant to buy/sell items (health potions, weapons, armor).
- Implement multiple saving and loading game states.
- Add a crafting system for combining items.
- Introduce a quest system with NPCs and dialogue.
- Fix any bugs related to health potion usage and inventory management.
- Enhance the GUI with images and better layout.
- On new game, wipe previous save data to prevent loading old characters, & map state.
- Add sound effects and background music.
- More balance between character classes and enemies.(Mage vs Warrior vs Rogue)
- Improve combat mechanics with special abilities and status effects.
- make usesable on devices with smaller screens.(calculators, phones)
- make inventory manageable with categories and sorting and more dynamically. (e.g., weapons, potions, quest items, stable elements)
...
class Character:
    def __init__(self, name=None, char_class=None, strength=0, intelligence=0, agility=0,
        ...
        self.inventory = inventory if inventory else ["Health Potion", "Health Potion", "Silver Sword", "The Oracleum"]
        self.gold = gold
        self.xp = xp
        self.level = level
...
class MutusMagicGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Mutus Magic RPG")

        self.main_frame = tk.Frame(root, padx=20, pady=20, bg="#EEFF00")
        self.main_frame.pack(fill="both", expand=True)

        self.output_text = tk.Text(self.main_frame, height=15, bg="#353535", fg="White")
        self.output_text.pack(fill="both")

        self.choice_frame = tk.Frame(self.main_frame, bg="#EEFF00", pady=2)
    ...
    def init_game(self):
        self.write("Welcome to Mutus Magic!")
        self.clear_choices()
        tk.Button(self.choice_frame, text="New Game", command=self.new_game).pack(side="left", padx=1)
        tk.Button(self.choice_frame, text="Load Game", command=self.load_game).pack(side="right", padx=1)

    def new_game(self):
        self.clear_choices()
        self.write("Enter your character name below:\n")
        self.name_entry = tk.Entry(self.choice_frame)
        self.name_entry.pack(side="top", padx=5)
        tk.Button(self.choice_frame, text="Next", command=self.choose_class).pack(side="bottom", padx=5)
    ...
    def choose_class(self):
        ...
        self.clear_choices()
        self.write(f"Name: {self.character_name}\n")
        self.write("Choose your Character class below:\n")
        for cls in ["Warrior", "Mage", "Rogue"]:
            tk.Button(self.choice_frame, text=cls, command=lambda c=cls: self.assign_attributes(c)).pack(side="left", padx=1)

    def assign_attributes(self, char_class):
        self.char_class = char_class
        self.clear_choices()
        self.write(f"Class: {self.char_class}\n")
        self.write("Distribute 15 points between: (Cannot respec.)\n Strength:  Intelligence:  Agility: \n")
        ...
        for attr in self.attr_vars:
            ...
            tk.Spinbox(row, from_=0, to=15, textvariable=self.attr_vars[attr], width=3, command=self.update_points).pack(side="left")
    ...
    def create_character(self):
        ...
        self.clear_choices()
        self.show_xp_bar()
        self.write(f"Character: {self.character.name}, the {self.character.char_class}")
        self.write(f"Level:({self.character.level}), XP:({self.character.xp}), Gold:({self.character.gold})")
        ...
        self.write("Your adventure begins now! There is no turning back.\n")
        self.show_main_menu()  # shown only if a boss is already defeated
    ...
    def show_main_menu(self):
        ...
        tk.Button(self.choice_frame, text="Explore Oraculum", command=self.explore_Oraculum).pack(side="left", padx=5)
    ...
    def show_inventory(self):
        ...
        self.show_xp_bar()
        self.write(f"Health: {self.character.current_health}/{self.character.max_health}  |  Gold: {self.character.gold}")
        self.write("\nInventory:")
    ...
    def explore_Oraculum(self):
        self.clear_choices()
        self.write("--------------------------------- Oraculum ----------------------------")
        self.write(".------..------..------..------..------.     .------..------..------.")
        self.write("|N.--. ||E.--. ||V.--. ||E.--. ||R.--. |.-.  |D.--. ||I.--. ||E.--. |")
        self.write("| :(): || (\/) || :(): || (\/) || :(): ((V)) | :/\: || (\/) || (\/) |")
        self.write("| ()() || :\/: || ()() || :\/: || ()() |'-.-.| (__) || :\/: || :\/: |")
        self.write("| '--'N|| '--'E|| '--'V|| '--'E|| '--'R| ((1)) '--'D|| '--'I|| '--'E|")
        self.write("`------'`------'`------'`------'`------'  '-'`------'`------'`------'")
        self.write("`~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-X-~~~~~~~~~~~~~~~~~~~~~~~~'")
        self.write("--------------------------------- Oraculum -----------------------------")
    ...
    def attack_boss(self, boss, location_data, callback):
        ...
        if enemy_roll >= 16:
            take_damage = random.randint(1, 25)
            self.character.take_damage(take_damage)
            self.write(f"{boss['name']} hits you for {take_damage} damage!")
        else:
    ...
    def sell_items(self):
        ...
        sellable_items = [item for item in self.character.inventory if item not in ["Silver Sword", "The Oracleum"]]
